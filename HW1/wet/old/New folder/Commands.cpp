...
/// </summary>
/// <param name="s">does not chang string</param>
...
void JobsList::addJob(Command* cmd, bool isStopped) {
        ...
        if (this->jobs && this->size > 0) {
            ...
            for (int i = 0; i < run_times; i++)
            {
                ...
                while (iter != this->jobs->end()) {
                    ...
                    if (res < 0) {
                        if (errno == ECHILD) {
                            if ((*iter)->geJobtId() == this->max_job_id) {
                                update_max = true;
                            }
        ...
        if (this->jobs && this->jobs->size() > 0)
        {
            ...
            for (auto it = this->jobs->begin(); it != this->jobs->end(); it++) {
                if ((*it)->geJobtId() == jobId) {
                    if ((*it)->geJobtId() == this->max_job_id) {
                        update_max = true;
                    }
                    this->jobs->erase(it);
                    this->size--;
    ...
    void ForegroundCommand::execute() {
        ...
        //child
            ...
            setpgrp();
            //moving command to fg by telling the proccess to continue
