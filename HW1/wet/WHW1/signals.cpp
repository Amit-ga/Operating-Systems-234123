...
void ctrlZHandler(int sig_num) {
    ...
    int to_terminate = 0;
    bool exists = false;
    JobsList::JobEntry* to_terminate_job = nullptr;
    ...
    if (sh.getRunningFgCmd()) {
        ...
        //check if already exists in jobs list
        if (sh.getAllJobs() && sh.getAllJobs()->size > 0) {
            for (auto it = sh.getAllJobs()->jobs->begin(); it != sh.getAllJobs()->jobs->end(); it++) {
                if ((*it)->getJobCommand()->getPID() == to_terminate) {
                    exists = true;
                    to_terminate_job = (*it);
                }
            }
        ...
        if (exists) {
            to_terminate_job->setIsJobStopped(true);
...
void ctrlCHandler(int sig_num) {
  // TODO: Add your implementation
    ...
    if (sh.getRunningFgCmd()) {
        //get command pid
        to_terminate = sh.getRunningFgCmd()->getPID();
        //check if proccess is still running before killing it
