...
void orderListInsert(MallocMetadata *meta) {
    ...
    while (iter_curr != nullptr) {
        if (iter_curr->cookie != random_cookie) {
            exit(0xdeadbeef);
        }
...
MallocMetadata *splitBlocks(MallocMetadata *to_split) {
    ...
    if(to_split->cookie != random_cookie)
    {
        exit(0xdeadbeef);
    }
    if (to_split->order == 0)
